...
import june1.open.common.exception.auth.IllegalTokenException;
...
import static june1.open.common.ConstantInfo.CODE_AUTH;
import static june1.open.common.ConstantInfo.SEQ;
...
public class TokenProvider {
    ...
    //해당 함수를 호출하기 전에 isValidToken() 함수를 호출할 것..
    public Long decryptToken(String token) {
        //토큰에서 필수 정보(사용자의 고유번호)를 꺼내기
        if (getClaims(token).get(SEQ) == null) {
        ...
        return Double.valueOf(getClaims(token).get(SEQ).toString()).longValue();
    }
