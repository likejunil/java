...
import june1.open.common.exception.auth.IllegalChangeGradeException;
import june1.open.common.exception.client.NoSuchCompanyException;
import june1.open.common.exception.client.NoSuchFileException;
...
import june1.open.common.util.FileUtil;
...
import june1.open.repository.AttachmentFileRepository;
...
public class MemberService {
    ...
    public QueryMemberResDto query(JwtUserInfo user, Long seq) {
        ...
        return QueryMemberResDto.builder()
                ...
                .role(m.getRole().name())
                .imageSeq(m.getImage() != null ? m.getImage().getId() : null)
                .build();
    }
    ...
    public MemberResDto modify(JwtUserInfo user, ModifyMemberReqDto dto) throws IOException {
        ...
        if (dto.getImage() != null && !dto.getImage().isEmpty()) {
            ...
            log.info("사용자[{}]의 파일을 새롭게 교체했습니다.[{}]=>[{}]",
                    user.getUserId(), m.getImage() != null ? m.getImage().getId() : null, f.getId());
     ...
     * @return
     */
    public FileDownDto downloadFile(Long seq) throws MalformedURLException {
        AttachmentFile f = attachmentFileRepository.findById(seq)
                .orElseThrow(() -> {
                    log.error("[{}]파일이 존재하지 않습니다.", seq);
                    return NoSuchFileException.builder()
                            .code(CODE_FILE)
                            .message("파일이 존재하지 않습니다.")
                            .object(object)
                            .field("downloadFile().seq")
                            .rejectedValue(String.valueOf(seq))
                            .build();
                });

        UrlResource resource = new UrlResource("file:" + f.getStoredFileFullName());

        return FileDownDto.builder()
                .name(f.getName())
                .resource(resource)
                .build();
    }
